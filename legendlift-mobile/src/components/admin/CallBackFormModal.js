...
const CallBackFormModal = ({ visible, onClose, onSubmit, callback = null, preSelectedCustomer = null, loading = false }) => {
  ...
  useEffect(() => {
    if (callback) {
      setCustomerId(callback.customer_id || '');
      setScheduledDate(callback.scheduled_date ? new Date(callback.scheduled_date) : new Date());
  ...
  }, [callback, preSelectedCustomer]);

  const fetchActiveCustomers = async () => {
    try {
      const response = await fetch(`${API_CONFIG.BASE_URL}/customers/active-amc/`, {
        headers: {
      ...
      Alert.alert('Error', 'Failed to load active AMC customers');
    }
  };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>
            {isEdit ? 'Edit CallBack' : 'Create CallBack'}
          </Text>
            ...
            <>
              ...
              <View style={styles.section}>
                ...
                <View style={styles.pickerContainer}>
                  <Text style={styles.label}>Select Customer (Active AMC Only) *</Text>
                  <View style={[styles.pickerWrapper, errors.customerId && styles.errorBorder]}>
                    ...
                    >
                      ...
                      {activeCustomers.map((customer) => (
                        <Picker.Item
                          key={customer.id}
                          label={`${customer.name} (${customer.job_number})`}
                          value={customer.id}
                        />
                      ))}
                    </Picker>
              ...
              {/* CallBack Details */}
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>CallBack Details</Text>
                ...
                <Input
                  ...
                  placeholder="Enter callback description"
                  multiline
                  numberOfLines={3}
              ...
              {/* Submit Button */}
              <Button
                title={isEdit ? 'Update CallBack' : 'Create CallBack'}
